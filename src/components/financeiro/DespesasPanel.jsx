...
export default function DespesasPanel() {
  ...
  const handleSave = async () => {
    try {
      ...
      const payload = {
        tipo: form.tipo,
        profile_id:
          form.tipo === "comissao_corretor" ? form.profile_id : null,
  ...
  const marcarComoPago = async (d) => {
    try {
      await fetch("/api/financeiro/despesas", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: d.id, status: "pago" }),
      });
  ...
  const dadosFiltrados = dados.filter((d) => {
    ...
    if (filters.dataInicio && d.data_vencimento < filters.dataInicio)
      return false;
    if (filters.dataFim && d.data_vencimento > filters.dataFim)
      return false;
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
        ...
        >
          ...
          <option value="repasse_proprietario">Repasse</option>
